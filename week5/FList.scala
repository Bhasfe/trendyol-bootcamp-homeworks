...
sealed trait FList[+A] {
  ...
  def :+[AA >: A](element: AA): FList[AA] = append(element)
  ...
  def +:[AA >: A](element: AA): FList[AA] = prepend(element)
  ...
  def ++[AA >: A](that: FList[AA]): FList[AA] = concat(that)
  ...
  def get(index: Int): Option[A] =
    this match {
      case FNil => None
      case FNonNil(head, tail) if index == 0 => Some(head)
      case FNonNil(head, tail) => tail.get(index - 1)
      }
  ...
  def filter(predicate: A => Boolean): FList[A] =
    this match {
      ...
      case FNonNil(head, tail) => tail.filter(predicate)
    }
}
